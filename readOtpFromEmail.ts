 ...
 * description: Get current date and save in YYYY-MM-DD format
 * actionType: custom_get_current_date
 * context: shared
 * needsLocator: false
 * category: Utilities
 */
export async function getCurrentDate(ctx: WalnutContext) {
  const now = new Date();
  ...
  ctx.log(`Current date saved to variable 'currentDate': ${currentDate}`);
}

/** @walnut_method
 * name: Read OTP from MS Mail
 * description: Read OTP from Salesforce verification email and save to variable
 * actionType: custom_read_otp_email
 * context: shared
 * needsLocator: false
 * category: Email Automation
 */
export async function readOtpFromEmail(ctx: WalnutContext) {
  const clientId = ctx.params.clientId || '958ef5ed-0ddd-4f7c-b41c-ad1c3572a21e';
  const clientSecret = ctx.params.clientSecret || 'a33fa700-774f-482a-a620-74d2014fd25c';
  const tenantId = ctx.params.tenantId || 'faa3f9fc-9b37-406b-b37d-58a9f045c17a';
  const emailAddress = ctx.params.emailAddress || 'shreya.g@simplify3x.com';
